...
/**
 * 研修コースの関連資料を管理者が追加・削除するモーダル。
 * AdminMaterialsModal（設定タスク用）と同じUIを、研修コース（courseId）向けに提供する。
 */
export default function AdminCourseMaterialsModal({ course, onClose, onAdd, onDelete }) {
  ...
  const isFull = materials.length >= MAX_MATERIALS_PER_ITEM;
  ...
  async function handleAdd(e) {
    ...
    setError(null);
    try {
      await onAdd(course.courseId, name.trim(), url.trim());
      setName("");
      setUrl("");
    } catch (err) {
  ...
  async function handleDelete(index) {
    ...
    setError(null);
    try {
      await onDelete(course.courseId, index);
    } catch (err) {
      setError(err.message || String(err));
    } finally {
      setBusy(false);
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
        ...
        {materials.length === 0 ? (
          <p className="hint">このコースにはまだ資料が添付されていません。</p>
        ) : (
          materials.map((m, i) => (
        ...
        {isFull ? (
          <div className="material-limit-note" style={{ color: "var(--danger)", fontWeight: 700 }}>
            上限の{MAX_MATERIALS_PER_ITEM}件に達しているため、これ以上追加できません。追加するには既存の資料を削除してください。
          </div>
        ) : (
          <form className="add-material-form" onSubmit={handleAdd}>
            <input
              placeholder="資料名（例：商材資料.pdf）"
              value={name}
