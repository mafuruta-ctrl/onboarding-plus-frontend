...
// 「AI整形する」やメンター面談レポート生成（Gemini呼び出しを含む）は、単純な
// シート読み書きより応答に時間がかかることがあるため、フロント側のタイムアウトに
// 余裕を持たせている（以前は15秒で、Gemini呼び出しが正常に動くようになった途端
// GAS_REQUEST_TIMEOUTになってしまう不具合があった）。
...
// Apps ScriptのWebアプリは、こちらのdoGetのコードが動き出す前の段階（Google側で
// 実行環境を用意して呼び出すまでの部分）でまれに詰まり、script.onerror
// （GAS_NETWORK_ERROR）やタイムアウト（GAS_REQUEST_TIMEOUT）になることがある。
// 実際に「失敗した直後に開き直したらすぐ成功する」ことが多いと確認できたため、
// この2種類のエラーに限り、少し間隔を空けて自動でリトライする。
// （result.okがfalseの業務エラーなど、GAS側のコードは動いたが処理内容として
// 失敗したケースはリトライしても無意味なので対象外。）
const RETRYABLE_ERROR_MESSAGES = ["GAS_NETWORK_ERROR", "GAS_REQUEST_TIMEOUT"];
const RETRY_DELAYS_MS = [1500, 3000]; // 1回目の失敗後1.5秒待って再試行、2回目の失敗後3秒待って再試行（最大3回試行）
...
async function jsonpRequestWithRetry(baseUrl, params) {
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
    try {
      ...
      lastError = err;
      const isRetryable = err && RETRYABLE_ERROR_MESSAGES.includes(err.message);
...
/**
 * GASのdoGetを1回呼び出す共通関数（通信エラー・タイムアウト時は自動リトライあり）。
 * result.ok が false の場合はGAS側のエラーメッセージでthrowする。
 */
async function callGasApi(action, idToken, params = {}) {
  const baseUrl = getBaseUrl();
  const result = await jsonpRequestWithRetry(baseUrl, { action, idToken: idToken || "", ...params });
...
export const gasApi = {
  /** サインイン確認だけ行う（action未指定でも動くヘルスチェック用途）。通信エラー時は自動リトライする。 */
  health(baseUrlOverride) {
    return jsonpRequestWithRetry(baseUrlOverride || getBaseUrl(), {});
  },
  ...
  submitMentorInterview(idToken, targetId, conductedAt, content, impression = "", hrQuestion = "") {
